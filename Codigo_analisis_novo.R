...
data <- data %>% 
  mutate(
    ...
    win.share = ifelse(win_top == "PAN", PAN.s,
...
p1 <- ggplot(data %>% filter(!is.na(alt)) , aes(x = as.factor(year), y = (win.share * 100), col = factor(alt))) + 
...
d1 <- ggplot(data %>% filter(!is.na(alt)) , aes(x = as.factor(year), y = (win.share * 100), col = factor(alt))) + 
  geom_boxplot() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
d1
...
a7a <- ggplot(data %>% filter(!is.na(alt) & ch.agua >= -25 & ch.agua <= 25 & win_top != "Otros"), aes(x = (win.share * 100), y = ch.agua, col = as.factor(win_top))) +
  geom_point(aes(alpha = 0.2)) +
